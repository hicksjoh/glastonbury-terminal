 ...
 * Runs behavioral checks, Kelly position sizing, and regime-aware recommendations
 * before any order hits Alpaca (paper or live).
 */

interface TradeGuardRequest {
  symbol: string;
  side: 'buy' | 'sell';
  quantity: number;
  price: number;
...
export async function POST(req: NextRequest) {
  try {
    const body: TradeGuardRequest = await req.json();
    ...
    const tradeContext: TradeContext = {
      ...
      ticker: symbol,
      quantity,
      recentOrderCount5Min: body.recentOrderCount5Min ?? portfolio.recentOrderCount5Min,
      vixLevel: vix ?? undefined,
      stockChangeLast5Days: body.stockChangeLast5Days ?? stockChange5D ?? undefined,
      wasOnWatchlist: body.wasOnWatchlist ?? false,
    ...
    const kellyInput: KellyInput = {
      expectedReturn: body.avgWin ? body.avgWin * (body.winRate || 0.5) - (body.avgLoss || 0.05) * (1 - (body.winRate || 0.5)) : 0.03,
      winRate: body.winRate || 0.55,
      avgWin: body.avgWin || 0.08,
      avgLoss: body.avgLoss || 0.05,
    };
